...
namespace ZJW.Common.DB
{
    public static partial class SqlMapperExtensions
    {
        ...
        {
            return connection.Insert<T>(entityToInsert, false, transaction, commandTimeout);
        }

        public static long InsertIdentity<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            return connection.Insert<T>(entityToInsert, true, transaction, commandTimeout);
        }
        ...
        private static long Insert<T>(this IDbConnection connection, T entityToInsert, bool withIdentity, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var isList = false;
            ...
            if (!isList)    //single entity
            {
                returnVal = adapter.Insert(connection, transaction, commandTimeout, name, sbColumnList.ToString(),
                    sbParameterList.ToString(), keyProperties, entityToInsert, withIdentity);
            }
            else
            {
            ...
            if (returnVal <= 0)
            {
                ...
                if (key != null)
                {
                    ...
                    long.TryParse(keValue.ToString(), out returnVal);
                }
            }
            return returnVal;
        }
...
{
    long Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert, bool withIdentity);
...
{
    public long Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert, bool withIdentity)
    {
        long id = 0;
        ...
        {
            id = connection.Execute(cmd, entityToInsert, transaction, commandTimeout);
        }
